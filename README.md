...
```
x_{k+1} = A x_k + u_k ,   k = 0, ..., N-1,   N = 25
```

where `u_k` is the external control applied at step `k` and the input matrix is the identity, so the manipulator can in principle address any node. Unrolling the recursion over the horizon gives `x_N = A^N x_0 + sum_k A^{N-k} u_k`. Writing `b = x_N - A^N x_0` and stacking the per-step controls as `u = [u_1; ...; u_N]` turns the schedule into a single linear system

```
b = Phi u ,   Phi = [ A^{N-1} | A^{N-2} | ... | A | I ]  (25 x 625)
```
...
The dataset is `data/PiecewiseSparse.mat`, with two arrays: `A` (25 x 25) and `FinalState` (1000 x 25), giving 1000 target final states. `A` is dense and signed, not symmetric and not row-stochastic, with spectral radius about 6.08 and complex eigenvalues. With only `A` and the final states supplied, the initial state is taken as `x_0 = 0`, so each row of `FinalState` is a target `b`.
...
- **l1 / basis pursuit (with BPDN for the noisy case)** — the convex baseline, `min ||u||_1` subject to `Phi u = b`. It is a different algorithm class with a different failure mode, and it cannot be told a per-block budget (Candes et al., 2006).
...
- **Conditioning is the load-bearing numerical issue.** Because `A` has spectral radius about 6.08, the column scales of `Phi` span roughly eighteen orders of magnitude, from about `2.55e18` in the earliest block (`A^24`) down to `1.0` in the identity block, and the raw dictionary has condition number about `5.14e16` (numerically singular). Normalizing the columns to unit norm brings the condition number to `16.67`. Without normalization the first greedy atom lands in the earliest block for all 100 sampled targets, with correlations about `5e17` times larger there purely by column scale; after normalization the atom is chosen on relevance instead.
- **Feasibility is essentially free.** With 25 equations and up to 25 nonzeros, even a budget of one active node per step (`s = 1`) reaches any target, with a median reconstruction residual of about `6.7e-10`. The budget is therefore an energy and timing knob, not a feasibility knob.
